...
The installed query builder serializes a transaction query into a `QueryState` containing `table`, `tableOptions`, filter/select/group/order expressions, calculation/raw/dead/reference flags, and nullable `limit` and `offset`. The v0.6.0 compiler fixes the table to `transactions` and selects only canonical fields: `id`, `account`, `date`, `amount`, `payee`, `payee.name`, `imported_payee`, `category`, `category.name`, `notes`, `imported_id`, `transfer_id`, `cleared`, `reconciled`, `starting_balance_flag`, `is_parent`, `is_child`, and `parent_id`.
...
## Transfer and reconciliation boundary

The installed `APIPayeeEntity` exposes `transfer_acct`, and `getPayees()` returns ordinary and transfer payees through the same public method. `ImportTransactionEntity.payee` accepts an existing payee ID and explicitly takes precedence over `payee_name`. Supplying an official transfer payee to the public transaction pipeline is therefore the supported way to request Actual's own reciprocal-transfer behavior.

`addTransactions(accountId, transactions, { runTransfers: true })` is public and returns the literal acknowledgement `"ok"`; it does not return created transaction IDs. The MCP must discover new IDs from exact before/after reads, verify reciprocal `transfer_id` values and opposite signed amounts, and must not treat the acknowledgement alone as proof of creation. `importTransactions` accepts the same optional `payee` field and returns `added`, `updated`, `updatedPreview`, and sanitized errors. The wrapper's default `runTransfers` value is false, so production code supplies true explicitly for manual transfer creation.

`updateTransaction` may return multiple affected rows, including relationship evidence, and `deleteTransaction(id)` returns transaction rows while the installed transfer handler can remove the reciprocal side. Transfer-aware mutation code must therefore preflight and read back both exact IDs. Canonical records preserve absent and explicit-null `transfer_id` independently; no counterpart is fabricated from a payee alone.

`getAccountBalance(id, cutoff?: Date)` publicly supports an optional cutoff date and returns one signed integer balance. It does not expose cleared/reconciled breakdowns. `APIAccountEntity` exposes optional nullable `balance_current`, but deliberately omits the internal account model's `last_reconciled`; bank-reported current balance is metadata, not a cutoff or statement balance.

The installed bundle contains internal transaction-merge and transfer helpers, but the public package exports no method to link two existing transactions, merge transactions, set reciprocal identifiers directly, lock or unlock reconciliation, or read/update last-reconciled state through `APIAccountEntity`. `ImportTransactionsOpts` also has no `payeeNameNormalization` option. These absent capabilities are unsupported rather than accessed through internal handlers, raw ActualQL, or SQLite.
