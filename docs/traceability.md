...
| Installed transfer payees, explicit `runTransfers`, import payee precedence, reciprocal IDs, transfer deletion, cutoff balance, and absent relationship/reconciliation exports | `test/contract/sdk-surface.test.ts`, `test/contract/actual-contract.test.ts`, `docs/actual-api-26.8.1-contract.md` |
...
| Canonical transfer projection, reciprocal pair integrity, direction, keys, de-duplication, sorting, lookup, search, and safe creation phases | `test/transfers.test.ts`, `test/transfer-client.test.ts`, guarded integration/E2E suites |
| Possible-transfer graph degree, duplicate evidence, null safety, bounds, counts, sorting, and pagination | `test/transaction-diagnostics.test.ts`, guarded integration/E2E read suites |
| Split-safe ledger/cleared/reconciled/uncleared aggregation, statements, bank metadata, and cutoff cross-check | `test/reconciliation.test.ts`, guarded integration/E2E read suites |
...
| Reciprocal transfer pair ownership, delete-once cleanup, exact absence verification, and reciprocal-ID fingerprints | `test/resources.test.ts`, `test/fingerprint.test.ts`, `test/real/` |
